...
<?php
	...
	include 'traitment_lettre.php';
		...
		if (empty($nBoncmd) || $nomSociete==0|| $objetMarche==0 ) {
			$_SESSION['errorMessage']='Remplir tout les champs';
			redirect_to("AddNewLettre.php");
		}
		elseif ($nBoncmd===0) {
			$_SESSION['errorMessage']='Le numero doit étre supérieur de 0';
			redirect_to("AddNewLettre.php");
		}
		else
		{	# QUERY FOR INSERTING DATA TO DATABASE
			$sql="INSERT INTO `commandetb`(`numero_cmd`, `date_cmd`, `ID_f`, `ID_marche`) VALUES(:zncmd,now(),:zidf,:zidmarche)";
				$stmt=$connectDB->prepare($sql);
				$exec=$stmt->execute(array(
					'zncmd' =>$nBoncmd,
					'zidf' => $nomSociete,
					'zidmarche'=>$objetMarche
					));
			# CHECK IF QUERY HAS BEEN SUCCESSFULLY INSERTED
			if ($exec) {
					$_SESSION['succesMessage']='La lettre a été ajouté avec succes';
					redirect_to("AddNewLettre.php");			
			}
			else {
...
<body>
	<div class="container col">
		...
		<div class="right-side">
			<?php include 'formSocMarch.php';?>
			<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
				<div class="card-header">
					<h2 class="text-center">Ajouter une nouvelle lettre</h2>
				</div>
				<?php
				echo errorMessage();
				echo succesMessage();
				...
				<div class="card-body">
					<div class="form-group row">
							...
							<div class="col-lg-8 input-group">
								<input type="text" class="form-control" name="nBoncmd" id="nBoncmd" placeholder="Enter le numero du bon de commande..">
	                    	</div>
	               	</div>
	               	<div class="form-group row">
						<label class="col-lg-4 col-form-label" for="nomSoc">Nom de Societé <span class="text-danger">*</span></label>
						<div class="col-lg-8 input-group">							
							<select  class="form-control mr-3" name="nomSociete" id="nomSoc">
								...
								foreach ($societies as $societe) {
									echo "<option value='".$societe['ID_f']."'>".$societe['nomF']."</option>";
								}?>
								
							</select>
							<!-- Button trigger modal -->
							<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#AddNewSoc">
               		...
               		<div class="form-group row">
						...
						<div class="col-lg-8 input-group">
							<select  class="form-control mr-3" name="objetMarche" id="objetM">
								<option value="0" disabled selected>...</option>
								<?php
								...
								foreach ($marches as $marche) {
									echo "<option value='".$marche['ID_marche']."'>".$marche['objet_marche']."</option>";
								}?>
							</select>
							<!-- Button trigger modal -->
							<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#AddNewMarche">
							 <i class="fas fa-plus"></i>
							</button>
                    	</div>
               		</div>
					<div class="form-group row">
							<div class="col-lg-8 ml-auto">
	                            <button type="submit" name="ajouterLetter" class="btn btn-primary">Ajouter la lettre</button>
	                        </div>
					</div>
				</div>
			</form>
